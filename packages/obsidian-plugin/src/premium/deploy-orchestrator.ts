...
export class DeployOrchestrator {
  ...
  async runDeployment(repoName: string, snapshot: VaultSnapshot, label: string): Promise<void> {
    try {
      ...
      new Notice(
        `Vault Insights: Deployment Complete! Visit https://${this.owner}.github.io/${repoName}/ (It may take a few minutes to go live)`,
      );
    } catch (error: any) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const getRes = await this.request(
          `/repos/${this.owner}/${repoName}/contents/vaults/index.json`,
        );
        ...
        if (putRes.status === 409) {
          console.warn(
            `Conflict updating index.json (Attempt ${attempt}/${maxRetries}). Retrying...`,
          );
